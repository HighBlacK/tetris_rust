...
pub fn encrypt_saves(saveformat: String) -> SaveResult<Vec<u8>> {
    ...
    let encrypted: Vec<u8> = {
        let encryptor: Encryptor = age::Encryptor::with_user_passphrase(
            Secret::new(ENCRYPTION_KEY.to_owned())
        );

        let mut encrypted: Vec<u8> = vec![];
        let mut writer: StreamWriter<&mut Vec<u8>> = write_encrypted_bytes(
            encryptor,
            &mut encrypted
        )?;
...
fn _empty_encryted_string() -> Arc<[u8]> {
    if ENCRYPTION_KEY == "o2Os#XJo&c1IP2@p5TOv9z@Cl5qw?9XejR2M0Di#" {
        ...
        panic!(
            "The encryption key has been changed. The empty encrypted string needs to be updated."
        );
    }
}
...
) -> SaveResult<StreamWriter<&mut Vec<u8>>> {
...
fn reading_decrypted_bytes(
    reader: &mut StreamReader<&[u8]>,
    decrypted_bytes: &mut Vec<u8>
) -> SaveResult<usize> {
...
fn create_decryptor(buffer: &[u8]) -> SaveResult<Decryptor<&[u8]>> {
    match age::Decryptor::new_buffered(buffer) {
        ...
        Err(_error) => {
            return Err(
                Box::new(
                    SaveError::new(SaveErrorKind::CorruptSave, "save file corrupted".to_owned())
                )
            );
        }
    }
