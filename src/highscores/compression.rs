...
pub fn compress_saves<T>(serialized_save: T) -> SaveResult<Vec<u8>> where T: AsRef<[u8]> {
...
fn write_all_to_buffer<T>(
    buffer: T,
    operator: &mut flate2::write::ZlibEncoder<Vec<u8>>
) -> SaveResult<()>
    where T: AsRef<[u8]>
{
...
fn read_all_to_buffer(
    buffer: &mut Vec<u8>,
    operator: &mut flate2::read::ZlibDecoder<&[u8]>
) -> SaveResult<()> {
