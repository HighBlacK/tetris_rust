...
impl<T> HandleSaveError<T> for SaveResult<T> {
    fn cathegorize(self) -> SaveResult<T> {
        let error: T = match self {
            ...
            Err(mut err) => {
                let mut new_save_error: SaveError = SaveError::new(
                    SaveErrorKind::Null,
                    SaveErrorKind::Null.as_string()
                );
                ...
                match save_error.kind {
                    SaveErrorKind::CorruptSave => {
                        println!("{:?}", save_error.message);
                        delete_corrupt_saves(true, SAVE_PATH.to_owned()).cathegorize()?;
                        return Err(
                            Box::new(
                                SaveError::new(SaveErrorKind::Generic, "save deleted".to_owned())
                            )
                        );
                    }
...
fn delete_corrupt_saves(encryption: bool, path: String) -> SaveResult<bool> {
    let corrupt_save_prompt: String = String::from(
        "The save data in the file 'scores.sav' seems to be corrupted\n Do you want to delete the save data? (y/n)"
    );
...
pub fn try_saving_again(player_info: PlayerInfo) -> SaveResult<()> {
    ...
    loop {
        ...
        match user_answer.trim().to_lowercase().as_str() {
            "y" => {
                match add_highscores(player_info.clone()) {
                    Ok(()) => println!("Score successfully saved"),
                    Err(_) => {
                        try_saving_again(player_info).cathegorize()?;
                    }
                }
                break;
            }
